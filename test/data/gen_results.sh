...
list_files "*.migo" $@ |
	while read FILE; do
		...
		FILENAME="${BASENAME%.*}"
		cd "$DIR";
		docker run --rm -v "$PWD":/root/go nickng/gong:popl17ae Gong -A /root/go/"$FILENAME".migo |& tee "$FILENAME".gong || true
		...
		dune exec dlock -- -ds 2 -s -m "$FILENAME".migo &> "$FILENAME".dlock.ds2 || true
...
list_files "*.ccs" $@ |
	while read FILE; do
		...
		dune exec dlock -- -ds 2 -s -p "$(cat "$FILENAME.ccs" | tr '\n' ' ')" &> "$FILENAME".ccs.dlock.ds2 || true
